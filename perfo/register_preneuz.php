...
<html lang="en">
  ...
  <body class="">
    <div class="off-canvas-wrap">
      ...
      >
        BOOTCAMP ENTRE DIGIT-PRENEUZ
      </h2>
    </div>
    <div class="row">
        <div class='' style="background-size:cover; display flex; justify-content:center;">
            <center><img src="images/entrepreneuses.jpeg" style="width:75%; box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1), 0 4px 6px -2px rgba(0, 0, 0, 0.05);border-radius:  0.25rem;" class='img-fluid' alt="Bootcamp entre digiteuz" /></center>
                <div>
                    <h4 class="mt-2 text-center">PROGRAMME</h4>

                    <p class='text-center'>Entre Digit Preneuz est un programme de formation d'une journée conçu spécialement pour les femmes,<br> entrepreneures débutantes, ou aspirantes qui veulent bâtir un business numérique..</p>
...
if (isset($_POST['save'])) {
    ...
    $req = $conn->prepare(
        'INSERT INTO performe_db.preneuz SET nom = :nom, prenom = :prenom, datenaissance = :datenaissance, email = :email, phone = :phone, commune = :commune, form_survey = :form_survey, genre = :genre, projet_survey = :projet_survey, nomprojet = :nomprojet, tic_survey = :tic_survey, info_survey = :info_survey, projet_desc = :projet_desc, lien = :lien, reg_date = :reg_date'
    );
    $req->execute([
        'nom' => $nom,
        'prenom' => $prenom,
